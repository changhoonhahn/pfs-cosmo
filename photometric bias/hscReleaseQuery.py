...
def main():
    ...
    prefix  =   'database/%s%s/sql_1000' %(release_year,diffver)
    prefix2 =   'database/%s%s/tracts_1000' %(release_year,diffver)
    ...
    if doDownload:
        ...
        for ig,tractL in enumerate(tracts2):
            ...
            #if(ig<194): continue
            print('Group: %s' %ig)
            downloadTracts(args, prefix, release_version, credential, sql,ig,tractL)

    if doUnzip:
        for ig,tractL in enumerate(tracts2):
            print('unzipping group: %s' %ig)
            separateTracts(args, prefix,prefix2,ig,tractL)
    
    return

def separateTracts(args, prefix,prefix2,ig,tractL):
    infname     =   '%s_bias.%s'%(ig,args.out_format)
...
def downloadTracts(args, prefix, release_version, credential, sql, ig,tractL):
    tractStr    =   map(str,tractL)
    tname       =   "'{0}'".format("', '".join(tractStr))
    print(tname)
    job         =   None
    sqlU        =   sql.replace('{$tract}',tname)
    outfname    =   '%s_bias.%s'%(ig,args.out_format)
    outfname    =   os.path.join(prefix,outfname)
    if os.path.exists(outfname):
        print('already have output')
        return
    print('querying data')
    job         =   submitJob(args, release_version, credential, sqlU, args.out_format)
    blockUntilJobFinishes(args, credential, job['id'])
    print('downloading data')
    fileOut     =   open(outfname,'w')
    fileBuffer  =   fileOut.buffer
    download(args, credential, job['id'], fileBuffer)
    if args.delete_job:
        deleteJob(args, credential, job['id'])
...
def submitJob(args, release_version, credential, sql, out_format):
...
def jobStatus(args, credential, job_id):
...
def blockUntilJobFinishes(args, credential, job_id):
    max_interval = 0.5 * 60 # sec.
    interval = 1
    while True:
        time.sleep(interval)
        job = jobStatus(args, credential, job_id)
        if job['status'] == 'error':
...
def download(args, credential, job_id, out):
...
def deleteJob(args, credential, job_id):
